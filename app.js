...
function updateProgress(phaseNumber) {
  const percent = (phaseNumber / phases.length) * 100;
  document.getElementById("progress-fill").style.width = percent + "%";
  document.getElementById("progress-text").innerText = `Phase ${phaseNumber} of ${phases.length}`;
}
...
function savePhase4() {
  ...
  if (healthStatus === "retired" && statePension === "yes") {
    message = "✅ You are likely eligible for an S1 form.";
  } else if (healthStatus === "working") {
    message = "💼 You will normally need to contribute to the local healthcare system or hold private insurance.";
  } else {
...
function savePhase5() {
  ...
  if (!type || !budget || !style) return alert("Please complete all housing questions");

  const resultBox = document.getElementById("housing-result");
  resultBox.innerHTML = `🏠 You plan to ${type} with a budget of £${budget}/month in a ${style} area.`;
  updateProgress(5);
}
...
function getAIAdvice() {
  ...
  output.innerHTML = "🤖 AI advice feature coming soon!";
}
